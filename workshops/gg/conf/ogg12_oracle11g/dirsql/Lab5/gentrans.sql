...
SET VERIFY OFF;
SET DEFINE '&';
PROMPT How many orders?(Default = 100) ;
ACCEPT lpctr DEFAULT '100';
...
BEGIN
  ...
  FOR v_MainLoop IN 1 .. &lpctr LOOP
    ...
    SELECT CUSTOMERS_ID INTO v_NxtCust FROM NEXT_CUST;
    
    /* Generate a random customer number */
    v_RdmCust := v_NxtCust + 20;
    SELECT dbms_random.value(1, v_RdmCust) INTO v_RdmCust FROM dual;
    ...
    CASE v_CustId
     WHEN '0' THEN
         ...
         SELECT customers_id into v_CustId FROM next_cust;
         UPDATE next_cust SET customers_id = customers_id + 1;
         COMMIT;
...
COMMIT;
    ...
    SELECT MAX(orders_products_id) INTO v_OrdProdId from orders_products;
